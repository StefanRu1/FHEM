...
sub Wunderground_Initialize($) {
    ...
    $hash->{AttrList} =
"disable:0,1 disabledForIntervals do_not_notify:1,0 timeout:1,2,3,4,5 pollInterval:300,450,600,750,900,1800,3600 wu_lang:en,de,at,ch,nl,fr,pl wu_pws:1,0 wu_bestfct:1,0 stateReadings stateReadingsFormat:0,1 "
...
sub Wunderground_GetStatus($;$) {
    ...
    my $features = AttrVal( $name, "wu_features", "" );
...
sub Wunderground_SendCommand($$) {
    ...
    my $URL =
      "https://api.weather.com/v3/wx/forecast/daily/5day?apiKey=%APIKEY%%FEATURES%&%QUERY%&units=m&format=json";
	...
	Log3 $name, 2, "Wunderground $name: GET " . urlDecode($URL);
