...
# 18.10.2024 fill codeDay with the day value of the icon, comment dumper					stefanru
...
sub _RetrieveDataFromWU {
    ...
    else
    {
     ...
     $paramRef->{url} = $URL . 'v3/wx/forecast/daily/' . $self->{days} . 'day' . '?' . $options;
    }
...
sub _RetrieveDataFinished {
    ...
    if ( !$err ) {
        ...
        $self->{forecastFetchTime} = time() if ( not defined( $self->{forecastFetchTime} ) );
            ...
            # }
                    ...
                    and scalar keys %{ $data->{observations}[0] } > 0 )
                {
                    ...
                    $self->{cached}{current} = {
                        ...
                        'pressure'    => sprintf( "%.1f", $data->{$unit}{pressure} ),
                        'temperature' => sprintf( "%.1f", $data->{$unit}{temp} ),
                        'temp_c' => sprintf( "%.1f", $data->{$unit}{temp} ),
                        'wind_chill' => sprintf( "%.1f", $data->{$unit}{windChill} ),
                        'windGust' => sprintf( "%.1f", $data->{$unit}{windGust} ),
                        'wind' => sprintf( "%.1f", $data->{$unit}{windSpeed} ),
                        'wind_speed' => sprintf( "%.1f", $data->{$unit}{windSpeed} ),
...
sub _ErrorHandling {
    ...
    $self->{cached}{current_forecast_date_time} =
      _strftimeWrapper( "%a, %e %b %Y %H:%M", localtime( $self->{forecastFetchTime} ) );
